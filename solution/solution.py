...
_package_name, _, _module_name = __name__.partition(".")
if _module_name and _package_name not in sys.modules:
    package = types.ModuleType(_package_name)
    package.__path__ = []
    sys.modules[_package_name] = package
if _module_name:
    setattr(sys.modules[_package_name], _module_name, sys.modules[__name__])
...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_cost_estimate = (len(gpt4o_response.split()) / 0.75) / 1000 * COST_PER_1K_OUTPUT_TOKENS[OPENAI_MODEL]
    return { 
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        history = history[-6:]  # Keep last 3 turns (user+assistant pairs)
